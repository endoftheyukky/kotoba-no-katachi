    ...
    for (const t of titles) {
      ...
      try {
        ...
        out.push({ text: t.text, reading: t.reading ?? '', set: t.set, variant: 0, space: 'trace', mode: 'v4', grammar: 'uniform', gvariant: '',
          material: { density: +c.parametric.material.density.toFixed(3), fineness: +c.parametric.material.fineness.toFixed(3), onForm: +c.parametric.material.onForm.toFixed(3), onRing: +c.parametric.material.onRing.toFixed(3), onPage: +c.parametric.material.onPage.toFixed(3) }, grains: c.parametric.grains, params: c.parametric.kind === 'lattice' ? { kind: 'lattice', rows: p.rows, regularity: +p.regularity.toFixed(3), shear: +p.shear.toFixed(3), decay: +p.decay.toFixed(3), curl: +p.curl.toFixed(3) } : { kind: 'trace', closure: +p.closure.toFixed(3), corners: +p.corners.toFixed(3), opening: +p.opening.toFixed(3), eccentricity: +p.eccentricity.toFixed(3), tangency: +p.tangency.toFixed(3), branch: !!p.branch },
