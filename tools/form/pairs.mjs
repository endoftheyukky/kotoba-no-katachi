...
export default async function ({ evaluate, load }) {
  ...
  const data = await evaluate(`(async () => {
    ...
    ctx.fillText(set + ' set — each pair: v2c (left) | v3 composition space (right). Caption: anchor · operators taken (share of what the title asked)', pad, 26)
    for (const [i, t] of titles.entries()) {
      ...
      const after = C.compose(a, { grammar: 'auto', form: 'v3' })
      ...
      const ops = (after.form?.steps ?? []).filter((s) => s.achieved > 0).map((s) => s.op + (s.achieved < 1 ? '·' + s.achieved : '')).join(' ')
