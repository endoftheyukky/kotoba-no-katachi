...
const RULES = {
  ...
  ink: `every entry whose glyph is in the face: v1 glyph/parts islands as measured (keep: the island's box); alike: islands whose shapes on a 16 × 16 grid over their boxes agree (IoU ≥ ALIKE_SHAPE) and whose shares are within ALIKE_SHARE of the larger, grouped in island order; crossings: cells where a horizontal and a vertical run of ink each span CROSS_RUN of the ink box and reach CROSS_ARM of it on both sides, joined within CROSS_JOIN em`,
...
export async function generate({ reuse = false, workers = Math.max(1, cpus().length - 2) } = {}) {
  ...
  const manifest = {
    ...
    render: {
      ...
      measure: { module: 'src/glyph/metrics.ts', sha256: sha(readFileSync('src/glyph/metrics.ts')), pxPerEm: 200, canvas: 600, ink: 'alpha > 127', box: 'alpha > 8', islands: { module: 'src/glyph/parts.ts', sha256: sha(readFileSync('src/glyph/parts.ts')) } },
