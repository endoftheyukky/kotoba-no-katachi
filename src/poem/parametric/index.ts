/**
 * The page, generated from continuous parameters read from the title:
 *
 *   the figure    the title walked as one curve — line → arc → ring, with
 *                 corners, branches and eccentricity — and written `rows`
 *                 times, each writing a row of its own. At one row it is a
 *                 band or a path; at many with no turning, a grid; at many
...
export interface ParametricApplied {
  /** a figure of more than one row reads as a lattice */
  kind: 'trace' | 'lattice'
  params: TraceParams
...
export function parametricPage(
  ...
  const t = traceParams(a, m, rng, motifs, meaning)
  const p: TraceParams = { ...t.params }
  ...
  const mat = materialParams(a, m, figure.nucleus?.char ?? null, figure.nucleus?.grapheme, motifs, meaning)
  ...
  const mp = { ...mat.params, density: offered * recede }
