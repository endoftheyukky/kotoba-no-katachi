/**
 * Title → analysis → proposals → primary operation (+ modifiers) → material
 *       → one page, drawn as a figure (poem/parametric).
 *
 ...
 *     prominent the relation is, as language) and visualPotential (how far a
 *     page can make it a strong, still legible structure). The primary
 *     operation is the proposal allowed to be primary with the highest
 *     poeticPotential = √(both); variant n takes the (n+1)-th;
 ...
 *     subject;
 *   - the page is drawn from continuous parameters read from the title and
 *     its material (poem/parametric).
 * The seed only moves plastic (造形) decisions inside what was read.
 */
...
import { parametricPage } from './parametric'
import { basisOf, scopeOf } from './scope'
import type { Analysis, Composition, Material, OperationId, PoeticOperation, Proposal, Rejection, Unit } from './types'
 ...
 * title never writes (exogenous). A heuristic, not a principle of the work.
 */
 ...
 * would shut out a 0.80 structure in the ink for good. (Heuristics.)
 */
...
export async function analyze(input: TitleInput, segmenter?: Segmenter): Promise<Analysis> {
  ...
  // what derived marks can be written in beyond the title's own characters:
  // the vowels a reading reduces to, and what the lexicon relates to the title
  const derivable = [...new Set(derivableChars(own))].filter((c) => covers('sans', c))
...
/**
 * The poem of an analysed title: what the title offers, the operation it is
 * built around and the material that operation leaves, drawn as one figure on
 * the page (poem/parametric). `meaning` is what the title was read to mean
 * (language/semantic/axes.ts); without it the page is read from the writing
 * alone.
 */
export function compose(a: Analysis, meaning: Meaning | null): Composition {
  ...
  const primary = eligible[variant % eligible.length]
  ...
  // the page: the figure, the paper it stands on and the material it is made of
