...
export const absence: PoeticOperation = {
  ...
  propose(a) {
    ...
    const candidates = [
      ...negations.map((r) => ({ rs: 0.9, d: 0.8, at: r.graphemes, label: `negation「${a.tokens[r.token].surface}」` })),
      ...silences.map((m) => ({ rs: 0.6, d: 0.5, at: m.graphemes, label: `silence「${m.text}」` })),
      ...words.map((t) => ({
        rs: 0.6,
