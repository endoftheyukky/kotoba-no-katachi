...
入力欄は二つある。ことばの欄と、その下の「読み（任意）」の欄である（`src/main.ts`）。読みの欄が空でなければ、その値を reading として使う。読みの欄が受け付けるのは、かな・ー・・・空白だけである（生成記録のサーバーと同じ条件）。どちらの欄でも Enter（変換の確定は除く）か、読みの欄の右端の「つくる」で紙面をつくる。

読みの欄が空なら、ことばのあとに括弧で添えた読みも、これまでどおり受け付ける（`子供の城（こどものしろ）`）。この書き方は画面では案内していない。ことばの欄は次の正規表現で分ける。
