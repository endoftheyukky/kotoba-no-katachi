...
- 生成の記録（`functions/`）は `npm run dev:archive`（wrangler pages dev、
  ローカルDB）で動きます。記録の内容と扱いは `docs/archive.md`。
...
<!-- LICENSE: 作者の判断待ち（コードと作品を分ける案を提示中）。決定後にここと LICENSE ファイルを確定する。 -->

- **ソースコード**：（決定待ち）
- **作品**（作品名「ことばのかたち」、作品についての文章、作例・スクリーンショット・
  コンタクトシートなどの画像）：（決定待ち）
